...
def test_build_progress_pattern_outer_cube_bar(tmp_path, spy_tqdm):
    ...
    assert s_bar == s_plain
    assert spy_tqdm.calls > 0  # outer cube bar (inner per-file bars are off under cube_parallel)
    assert open_dataset(glob_path, pattern, metadata_caching=True,
                        metadata_cache_path=str(bar)) is not None


@pytest.mark.integration
def test_build_no_proactive_decode_probe_on_decodable(tmp_path, monkeypatch):
    # Fix B: a decodable dataset must NOT be opened a second time just to probe time units. The
    # probe is reactive now -- consulted only if an open actually fails -- so on the common
    # decodable path ``_time_units_undecodable`` is never called.
    files = _make_slabs(tmp_path)["nc4"]
    calls = []
    real = _kerchunk._time_units_undecodable
    monkeypatch.setattr(
        _kerchunk, "_time_units_undecodable",
        lambda f: (calls.append(f), real(f))[1],
    )
    summary = build_metadata_cache(files, metadata_cache_path=str(tmp_path / "c"))
    assert summary["built"] == 1
    assert calls == []  # zero probe opens on the decodable path


@pytest.mark.integration
def test_undecodable_time_units_reactive_fallback_without_pin(tmp_path, monkeypatch):
    # Fix B: with NO caller-pinned decode_times and non-CF "months since" time, the build opens
    # once (fails), confirms via the on-failure probe, then retries with decode_times=False --
    # succeeding transparently and persisting the flag so the reader replays it.
    p = _write_monthly(tmp_path / "monthly.nc")
    calls = []
    real = _kerchunk._time_units_undecodable
    monkeypatch.setattr(
    ...
    cache = str(tmp_path / "cache")
    summary = build_metadata_cache(p, metadata_cache_path=cache)  # no decode_times pin
    assert summary["built"] == 1 and summary["skipped"] == []
    assert calls  # probe consulted only on the failure path

    store = _kerchunk.load_store(cache)
    assert store["open_kwargs"].get("decode_times") is False  # auto-applied + persisted
    ds2 = _kerchunk.open_store(store)  # reader replays persisted decode_times=False (no kwargs)
    t = np.asarray(ds2["time"].values)
    assert not np.issubdtype(t.dtype, np.datetime64) and t.size == 6 and int(t[0]) == 0


@pytest.mark.integration
def test_pattern_cube_parallel_matches_serial(tmp_path):
    # Fix A: building the per-cube stores concurrently via the dask scheduler (cube_parallel,
    # inner opens forced serial -> no nested dask.compute) must produce the same per-cube stores
    # and index as the serial cube loop. "synchronous" exercises the delayed/compute submission
    # deterministically (no real-concurrency h5py races).
    glob_path = os.path.join("tests", "resources", "era5-single-levels-reanalysis_*.nc")
    pattern = os.path.join("tests", "resources", "era5-single-levels-reanalysis_{var}.nc")
    serial, par = tmp_path / "serial", tmp_path / "par"
    s_sum = build_metadata_cache(glob_path, pattern, metadata_cache_path=str(serial),
                                 cube_parallel=False)
    p_sum = build_metadata_cache(glob_path, pattern, metadata_cache_path=str(par),
                                 cube_parallel=True, scheduler="synchronous")
    assert p_sum == s_sum
    assert p_sum["groups"] >= 2 and p_sum["built"] == p_sum["groups"]  # genuinely multi-cube

    def _stores(root):
        cubes = root / _cache.CUBES_DIR
        return {g: _kerchunk.load_store(str(cubes / g)) for g in sorted(os.listdir(cubes))}

    s_stores, p_stores = _stores(serial), _stores(par)
    assert p_stores.keys() == s_stores.keys()
    for g in s_stores:
        assert p_stores[g]["partitions"] == s_stores[g]["partitions"]  # same consolidated refs
    assert (_cache.read_json(str(par / _cache.INDEX_FILE))
            == _cache.read_json(str(serial / _cache.INDEX_FILE)))  # identical index
    assert open_dataset(glob_path, pattern, metadata_caching=True,
                        metadata_cache_path=str(par)) is not None
