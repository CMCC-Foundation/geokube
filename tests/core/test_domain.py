...
def test_create_points_1(points1):    
    ...
    assert np.all(points1.coords[axis.latitude] == pint.Quantity([42.2, 56.2], units['degrees_north']))
...
def test_create_points_2(points2):    
    ...
    assert np.all(points2.coords[axis.latitude] == pint.Quantity([42.2, 56.2], units['degrees_north']))
    assert np.all(points2.coords[axis.longitude] == pint.Quantity([-12.2, 10.2], units['degrees_east']))
...
def test_nearest_horizontal_points(points1):
    ...
    assert pts.coords[axis.latitude] == pint.Quantity(42.2, units['degrees_north'])
    assert pts.coords[axis.longitude] == pint.Quantity(-12.2, units['degrees_east'])
...
def test_time_range_points(points1):
    ...
    assert pts.number_of_points == 1
    assert (
        pts.coords[axis.vertical] == pint.Quantity(11.2, units['meter'])
    )
    assert (
        pts.coords[axis.latitude]
        == pint.Quantity(56.2, units['degrees_north'])
    )
    assert (
        pts.coords[axis.longitude]
        == pint.Quantity(10.2, units['degrees_east'])
    )
    ...
    assert (
        pts.coords[axis.latitude]
        == pint.Quantity(42.2, units['degrees_north'])
    )
    assert (
        pts.coords[axis.longitude]
        == pint.Quantity(-12.2, units['degrees_east'])
    )
...
def test_create_profiles(profiles):    
    ...
    assert np.all(profiles.coords[axis.latitude] == pint.Quantity([42.2, 56.2], units['degrees_north']))
    assert np.all(profiles.coords[axis.longitude] == pint.Quantity([-12.2, 10.2], units['degrees_east']))
    assert np.allclose(
...
def test_time_range_profiles(profiles):
    ...
    assert np.allclose(
        prof.coords[axis.latitude].magnitude, np.array([56.2])
    )
    assert np.allclose(
        prof.coords[axis.longitude].magnitude, np.array([10.2])
    )
